...
def player1():
    ...
    if p1.isdigit() and len(p1) == 2:
        ...
        print("Нужно ввести 2 цыфры в диапазоне от 1 до 3")
        player1()
    var()
...
# завершения программы по окончании ходов
def nichya_chek():
    k = 0
    t = 0
    for i in range(7):
        if {'X', '0'} == set(line_all[i]):
            k =+1
    for j in range(7):
        if {'0', 'X'} == set(line_all[j]):
            t += 1
    if k + t >= 8:
        print('Ходы завершены, это ничья!')
        return False
    else:
        return True
